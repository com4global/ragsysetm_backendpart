...
async def avatar_video_dashboard(
    current_user: User = Depends(get_current_user),
    doc_name: str = Query("", description="Optional document name to filter/discover topics for"),
):
    """
    ...
    Accepts optional doc_name to filter by a specific document.
    """
    try:
        # 1. Get all topics from lesson_cache grouped by document
        documents = {}  # doc_name -> { topics: [...], ... }

        def _parse_topics_from_rows(rows):
            """Helper to parse topic rows from lesson_cache into documents dict."""
            for row in (rows or []):
                dn = row.get("doc_name", "Unknown")
                ...
                if dn not in documents:
                    documents[dn] = {"doc_name": dn, "topics": []}
                for t in topics_list:
                    if isinstance(t, dict):
                        ...
                        key_concepts = []
                    if title.strip():
                        existing_titles = [tp["title"].lower() for tp in documents[dn]["topics"]]
                        if title.strip().lower() not in existing_titles:
                            documents[dn]["topics"].append({
                                "title": title.strip(),
                                "description": desc,
        ...
        try:
            # First: query lesson_cache for this user's topics ONLY
            query = supabase.table("lesson_cache").select("doc_name, lesson_json, topic") \
                .eq("lesson_type", "topics").eq("user_id", current_user.id)
            if doc_name:
                query = query.eq("doc_name", doc_name)
            result = query.execute()
            _parse_topics_from_rows(result.data)

            # Fallback: if no topics found, check user_files to show the user's docs
            # (topics may not have been generated yet, but the docs belong to this user)
            if not documents:
                logger.info(f"Dashboard: No topics found for user {current_user.id}, checking user_files")
                files_result = supabase.table("user_files").select("filename") \
                    .eq("user_id", current_user.id).execute()
                for f in (files_result.data or []):
                    fn = f.get("filename", "")
                    if fn and fn not in documents:
                        documents[fn] = {"doc_name": fn, "topics": []}
        ...
        logger.error(f"Dashboard endpoint crashed: {e}", exc_info=True)
        return {
            "success": False,
...
def _discover_all_topics_from_db(user_id: str = None) -> list:
    """
    Discover unique topic titles from Supabase lesson_cache
    (lesson_type = 'topics'). When user_id is provided, only
    returns topics belonging to that user.
    """
    all_topics = set()
    try:
        # Query lesson_cache for 'topics' entries
        query = supabase.table("lesson_cache").select("lesson_json") \
            .eq("lesson_type", "topics")
        if user_id:
