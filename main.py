...
async def avatar_video_dashboard(
    ...
    try:
        ...
        try:
            # First: query lesson_cache for this user's topics
            ...
            # Fallback: if doc_name was specified but no results found for this user,
            # try without user_id filter (topics may have been generated by system/admin)
            if doc_name and not documents:
                logger.info(f"Dashboard: No topics found for user {current_user.id} + doc '{doc_name}', trying global search")
                fallback = supabase.table("lesson_cache").select("doc_name, lesson_json, topic") \
                    .eq("lesson_type", "topics").eq("doc_name", doc_name).limit(10).execute()
                _parse_topics_from_rows(fallback.data)
